...
begin
  ...
  if p_action='document.ack' then v_id:=(p_payload->>'documentId')::uuid; select team_id into v_team from team_documents where id=v_id; if app_team_role(v_team,u) is null then raise exception 'not_a_team_member'; end if; insert into document_acknowledgments(document_id,user_id) values(v_id,u) on conflict(document_id,user_id) do update set acknowledged_at=now(); return jsonb_build_object('ok',true,'acknowledgedAt',now()); end if;
  ...
  if p_action='conversation.create' then
    if v_role is null then raise exception 'not_a_team_member'; end if; if coalesce(p_payload->>'kind','direct') in ('team','coaches','event') and not app_is_coach(v_team,u) then raise exception 'coach_role_required'; end if;
  ...
  if p_action='conversation.message.send' then insert into messages(conversation_id,sender_user_id,ciphertext,nonce,crypto_version,client_message_id) values(v_id,u,decode(p_payload->>'ciphertext','base64'),decode(p_payload->>'nonce','base64'),left(coalesce(p_payload->>'cryptoVersion','v1'),80),left(p_payload->>'clientMessageId',160)) on conflict(sender_user_id,client_message_id) do update set client_message_id=excluded.client_message_id returning id,sent_at into v_id2,v_text; return jsonb_build_object('id',v_id2,'sent_at',v_text); end if;
  ...
  if p_action='form.submit' then
    v_id:=(p_payload->>'formId')::uuid; if not exists(select 1 from form_templates ft where ft.id=v_id and ft.team_id=v_team) then raise exception 'form_not_found'; end if;
