...
test('connectivity indicator is wired, accessible, active-team aware, and offline-cached',()=>{
  const html=read('index.html'),script=read('core/connectivity-status.js'),queue=read('core/cloud-queue.js'),css=read('core/connectivity-status.css'),sw=read('sw.js');
  ...
  assert.match(script,/TeamAppRuntime/);
  assert.match(script,/remoteId/);
  assert.match(script,/addEventListener\('online'/);
  assert.match(script,/addEventListener\('offline'/);
  assert.match(script,/addEventListener\('teamapp:queue-change'/);
  assert.match(queue,/teamapp:queue-change/);
