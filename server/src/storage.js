...
export async function prepareUpload({teamId,documentId,name,contentType,byteSize}){
  ...
  if(config.storageProvider==='s3'){
    ...
    const command=new PutObjectCommand({Bucket:config.s3Bucket,Key:key,ContentType:contentType||'application/octet-stream',...(Number.isFinite(Number(byteSize))?{ContentLength:Number(byteSize)}:{}),...encryption});
...
export async function inspectStoredFile(key){
  if(config.storageProvider==='s3'){
    const out=await s3Client().send(new HeadObjectCommand({Bucket:config.s3Bucket,Key:key}));
    return {size:Number(out.ContentLength||0),contentType:String(out.ContentType||'application/octet-stream')};
  }
  const st=await fs.stat(localPath(key));return {size:Number(st.size||0),contentType:null};
}
