...
const roles=['admin','coach','assistant_coach','manager','guardian','member','readonly'];
const inviteSchema=z.object({email:z.string().email(),role:z.enum(roles),athleteId:z.string().uuid().optional(),athleteClientKey:z.string().max(120).optional(),expiresHours:z.number().int().min(1).max(336).default(72)});
...
router.post('/teams/:teamId/invitations',requireTeamRole(ADMIN_ROLES),async(req,res,next)=>{try{
  ...
  let athleteId=body.athleteId||null;if(!athleteId&&body.athleteClientKey){athleteId=(await query(`select a.id from roster_memberships r join athlete_profiles a on a.id=r.athlete_id where r.team_id=$1 and a.client_key=$2`,[teamId,body.athleteClientKey])).rows[0]?.id||null;}
...
router.post('/invitations/accept',async(req,res,next)=>{try{
  ...
  const result=await tx(async c=>{
    ...
    await c.query(`insert into organization_memberships(organization_id,user_id,role) values($1,$2,$3) on conflict(organization_id,user_id) do update set role=case when case excluded.role::text when 'owner' then 100 when 'admin' then 90 when 'coach' then 80 when 'assistant_coach' then 70 when 'manager' then 65 when 'guardian' then 30 when 'member' then 20 when 'readonly' then 10 else 0 end>case organization_memberships.role::text when 'owner' then 100 when 'admin' then 90 when 'coach' then 80 when 'assistant_coach' then 70 when 'manager' then 65 when 'guardian' then 30 when 'member' then 20 when 'readonly' then 10 else 0 end then excluded.role else organization_memberships.role end`,[inv.organization_id,user.id,inv.role]);
...
const codeSchema=z.object({role:z.enum(['assistant_coach','manager','guardian','member','readonly']).default('guardian'),athleteId:z.string().uuid().optional(),athleteClientKey:z.string().max(120).optional(),maxUses:z.number().int().min(1).max(100).default(1),expiresHours:z.number().int().min(1).max(2160).optional()});
router.post('/teams/:teamId/join-codes',requireTeamRole(ADMIN_ROLES),async(req,res,next)=>{try{
  const body=codeSchema.parse(req.body);let athleteId=body.athleteId||null;if(!athleteId&&body.athleteClientKey){athleteId=(await query(`select a.id from roster_memberships r join athlete_profiles a on a.id=r.athlete_id where r.team_id=$1 and a.client_key=$2`,[req.params.teamId,body.athleteClientKey])).rows[0]?.id||null;}let code,hash;for(let i=0;i<5;i++){code=humanCode();hash=sha256(code);const exists=(await query('select 1 from team_join_codes where code_hash=$1',[hash])).rowCount;if(!exists)break;}
  const expires=body.expiresHours?new Date(Date.now()+body.expiresHours*3600000):null;
...
router.post('/join-codes/redeem',async(req,res,next)=>{try{
  ...
  const result=await tx(async c=>{
    ...
    await c.query(`insert into organization_memberships(organization_id,user_id,role) values($1,$2,$3) on conflict(organization_id,user_id) do update set role=case when case excluded.role::text when 'owner' then 100 when 'admin' then 90 when 'coach' then 80 when 'assistant_coach' then 70 when 'manager' then 65 when 'guardian' then 30 when 'member' then 20 when 'readonly' then 10 else 0 end>case organization_memberships.role::text when 'owner' then 100 when 'admin' then 90 when 'coach' then 80 when 'assistant_coach' then 70 when 'manager' then 65 when 'guardian' then 30 when 'member' then 20 when 'readonly' then 10 else 0 end then excluded.role else organization_memberships.role end`,[rec.organization_id,user.id,rec.role]);
