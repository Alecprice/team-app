...
router.post('/invitations/accept',async(req,res,next)=>{try{
  ...
  const result=await tx(async c=>{
    ...
    await c.query(`insert into team_memberships(team_id,user_id,role) values($1,$2,$3) on conflict(team_id,user_id) do update set role=excluded.role`,[inv.team_id,user.id,inv.role]);
    await c.query(`insert into organization_memberships(organization_id,user_id,role) values($1,$2,$3) on conflict(organization_id,user_id) do nothing`,[inv.organization_id,user.id,inv.role]);
...
router.post('/join-codes/redeem',async(req,res,next)=>{try{
  ...
  const result=await tx(async c=>{
    ...
    await c.query(`insert into team_memberships(team_id,user_id,role) values($1,$2,$3) on conflict(team_id,user_id) do update set role=excluded.role`,[rec.team_id,user.id,rec.role]);
    await c.query(`insert into organization_memberships(organization_id,user_id,role) values($1,$2,$3) on conflict(organization_id,user_id) do nothing`,[rec.organization_id,user.id,rec.role]);
