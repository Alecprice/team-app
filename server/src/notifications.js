...
async function markDelivered(teamId,category,result){
  const delivered=Array.isArray(result?.deliveredUserIds)?[...new Set(result.deliveredUserIds.filter(Boolean))]:[];
  if(result?.disabled||!delivered.length)return;
  await query(`update notification_events set sent_at=now() where team_id=$1 and user_id=any($2::uuid[]) and kind=$3 and sent_at is null and failed_at is null`,[teamId,delivered,category]);
}
...
export async function notifyUsers(userIds,teamId,category,{title,body,payload={}}){
  ...
  const result=await sendPushToUsers(ids,{title,body,payload:{teamId,category,...payload}});await markDelivered(teamId,category,result);return result;
}
export async function notifyTeam(teamId,category,{title,body,payload={}},excludeUserId=null){
  ...
  await markDelivered(teamId,category,result);
  return result;
}
