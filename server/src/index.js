...
app.get('/api/health',async(req,res)=>{try{await query('select 1');res.json({ok:true,service:'team-app',version:'1.10.0'});}catch(err){res.status(503).json({ok:false,error:'database_unavailable'});}});
...
app.use((err,req,res,next)=>{console.error('[server]',err);if(res.headersSent)return next(err);const requested=Number(err?.status),status=err?.name==='ZodError'?400:(Number.isInteger(requested)&&requested>=400&&requested<=599?requested:500);const safeCode=status<500&&/^[a-z0-9_:-]+$/i.test(String(err?.message||''))?String(err.message):(status===400?'invalid_request':status===403?'forbidden':status===404?'not_found':status===409?'conflict':'internal_error');res.status(status).json({error:safeCode,details:config.nodeEnv==='production'?undefined:err?.message});});

app.listen(config.port,()=>console.log(`Team APP V1.10 listening on ${config.authUrl}`));
