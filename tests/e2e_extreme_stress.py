...
def budget(timings,key): assert timings[key]<=MAX_NAV_MS,f'{key} exceeded {MAX_NAV_MS}ms budget: {timings[key]}ms'
def main():
 with sync_playwright() as p:
  ...
  for key,selector,count in [('roster','.roster-list .player-row',500),('practice','.timeline-item',200),('schedule','.event-card',250)]:
   t=time.perf_counter();page.locator(f'[data-nav="{key}"]').click();page.locator(selector).first.wait_for();timings[key+'_ms']=round((time.perf_counter()-t)*1000);assert page.locator(selector).count()==count;overflow(page,key);budget(timings,key+'_ms')
  t=time.perf_counter();page.locator('[data-nav="lineup"]').click();page.locator('[data-lineup-tab="rotation"]').click();page.locator('.rotation-table').wait_for();timings['rotation_ms']=round((time.perf_counter()-t)*1000);assert page.locator('.rotation-table tbody tr').count()==500;overflow(page,'rotation');budget(timings,'rotation_ms')
  t=time.perf_counter();page.locator('#gameDayBtn').click();page.locator('[data-open-game]').first.click();page.locator('.game-checkin-card').wait_for();timings['gameday_ms']=round((time.perf_counter()-t)*1000);assert page.locator('.checkin-row').count()==500;overflow(page,'gameday');budget(timings,'gameday_ms')
  state_bytes=page.evaluate("new Blob([JSON.stringify(window.__TEAM_APP_TEST__.snapshot())]).size")
  assert state_bytes<MAX_STATE_BYTES,f'serialized state {state_bytes} exceeded {MAX_STATE_BYTES} byte budget'
  assert not errors,errors;b.close()
 report={'status':'PASS','dataset':got,'timings':timings,'serialized_state_bytes':state_bytes,'budgets':{'max_navigation_ms':MAX_NAV_MS,'max_state_bytes':MAX_STATE_BYTES},'viewport':'390x844','page_errors':0}
