...
def html():
    return f'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>{(ROOT/'styles.css').read_text()}\n{(ROOT/'core'/'hardening-runtime.css').read_text()}</style></head><body><div id="app"></div><script>window.__TEAM_APP_ENABLE_TEST_HOOKS__=true;</script><script>{(ROOT/'sports.js').read_text()}</script><script>{(ROOT/'core'/'sport-runtime.js').read_text()}</script><script>{(ROOT/'core'/'hardening-runtime.js').read_text()}</script><script>{(ROOT/'app.js').read_text()}</script></body></html>'''
...
def audit(page,label):
    issues=page.evaluate('''() => {
      ...
      document.querySelectorAll('input,select,textarea').forEach((e,i)=>{
        ...
        if(!explicit&&!wrapping&&!e.getAttribute('aria-label')&&!e.getAttribute('aria-labelledby'))out.push(`${e.tagName.toLowerCase()}[${e.name||i}] missing programmatic label`);
      });
...
def main():
    with sync_playwright() as p:
      ...
      b.close()
    report={'status':'PASS','viewport':'320x568','checks':['button accessible names','programmatic form labels','image alt text','200% text-size horizontal overflow'],'surfaces':['home','roster','lineup','practice','schedule','learn','player modal','event modal','coach center']}
