const CACHE='team-app-live-v1.10.0-hardening-2';
...
const SENSITIVE_QUERY_KEYS=new Set([
  'invite','invitation','invite_token','invitetoken',
  'token','access_token','refresh_token','recovery_token',
  'code','join','join_code','joincode',
  'session','session_token','auth','authorization','secret'
]);
function hasSensitiveQuery(url){
  for(const key of url.searchParams.keys())if(SENSITIVE_QUERY_KEYS.has(String(key).toLowerCase()))return true;
  return false;
}
...
self.addEventListener('fetch',event=>{
  ...
  if(sensitive){
    event.respondWith(fetch(event.request).catch(()=>new Response('Offline asset unavailable',{status:503,statusText:'Offline'})));
    return;
  }
  event.respondWith(fetch(event.request).then(async res=>{
    if(res.ok){
    ...
    return res;
  }).catch(async()=>await caches.match(event.request)||new Response('Offline asset unavailable',{status:503,statusText:'Offline'})));
});
