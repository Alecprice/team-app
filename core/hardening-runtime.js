(function(root){
  ...
  function repairLabels(scope=document){
    scope.querySelectorAll?.('.field').forEach(field=>{const label=field.querySelector(':scope > label');const control=field.querySelector(':scope > input:not([type="hidden"]),:scope > select,:scope > textarea');if(!label||!control||label.contains(control)||control.getAttribute('aria-label')||control.getAttribute('aria-labelledby'))return;if(!control.id)control.id=`teamappField${++filterSeq}`;if(!label.htmlFor)label.htmlFor=control.id;});
  ...
  function hardenGuardianAccess(scope=document){
    const form=scope.querySelector?.('#inviteForm');
    if(form&&!form.dataset.guardianHardened){
      form.dataset.guardianHardened='1';const role=form.querySelector('[name="role"]'),athlete=form.querySelector('[name="athleteClientKey"]'),field=athlete?.closest('.field'),label=field?.querySelector('label');
      const refresh=()=>{const guardian=role?.value==='guardian';if(athlete){athlete.required=guardian;const empty=athlete.querySelector('option[value=""]');if(empty){empty.disabled=guardian;empty.textContent=guardian?'Choose athlete…':'No specific athlete';}if(guardian&&!athlete.value)athlete.value='';}if(label)label.textContent=guardian?'Guardian athlete link (required)':'Link to athlete (optional)';};
      role?.addEventListener('change',refresh);refresh();
    }
    const join=scope.querySelector?.('#joinAthleteSelect');
    if(join&&!join.dataset.guardianHardened){join.dataset.guardianHardened='1';join.required=true;const empty=join.querySelector('option[value=""]');if(empty){empty.disabled=true;empty.textContent='Choose athlete…';}if(!join.value)join.value='';}
  }

  function hardenDynamicUi(scope=document){
    repairLabels(scope);hardenGuardianAccess(scope);if(!DEMO)scope.querySelectorAll?.('#resetApp').forEach(el=>el.remove());scope.querySelectorAll?.('#documentForm select[name="category"] option').forEach(o=>{if(/medical\s*\/\s*safety/i.test(o.textContent||''))o.remove();});scope.querySelectorAll?.('.roster-list').forEach(el=>addListFilter(el,'players'));scope.querySelectorAll?.('.document-list').forEach(el=>addListFilter(el,'documents'));scope.querySelectorAll?.('.game-event-list').forEach(el=>addListFilter(el,'events'));scope.querySelectorAll?.('.badge-dot').forEach(el=>{if(!el.dataset.unread)el.hidden=true;});
  }
