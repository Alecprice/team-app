(() => {
  ...
  function renderCoachCenter(){
    ...
    const ruleUrl=t.ruleSourceUrl||profile?.sourceUrl||league?.sourceUrl||'';
  ...
  function littleLeaguePitchGuide(p,pitches,eventId){
    if(sportKey(team())!=='baseball'||team().leagueKey!=='little-league')return null;const age=Number(p?.leagueAge);if(!Number.isFinite(age)||age<6||age>16)return {missingAge:true};
    const daily=age<=8?50:age<=10?75:age<=12?85:95,rest=littleLeagueRestDays(age,pitches),remaining=Math.max(0,daily-pitches);
    const event=state.events.find(e=>String(e.id)===String(eventId));let eligibility=null;
    if(event?.date){const current=new Date(`${event.date}T12:00:00`),history=Object.entries(state.gameSessions||{}).map(([id,g])=>{const ev=state.events.find(e=>String(e.id)===String(id));const n=Number(g?.pitchCounts?.[p.id]||0);return ev?.date&&n>0&&String(id)!==String(eventId)?{id,date:new Date(`${ev.date}T12:00:00`),dateText:ev.date,pitches:n}:null;}).filter(Boolean).filter(x=>x.date<current).sort((a,b)=>b.date-a.date);
    ...
    return {age,daily,rest,remaining,eligibility,source:'https://www.littleleague.org/playing-rules/pitch-count/'};
  }
  ...
  function renderLearn(){
    ...
    return `<div class="page-head"><div><div class="eyebrow">${esc(sportDef().name)} education</div><h1>Learn the game</h1><p>Position, rule and best-practice education uses the same sport definition as the coach tools.</p></div></div>${unitTabs}
      <div class="notice" style="margin-bottom:14px"><strong>${esc(teamRuleLabel(team()))}${team().leagueName?` · ${esc(team().leagueName)}`:''}.</strong> Fundamentals are instructional. Competition rules are tied to the selected age/division and governing source.${team().ruleSourceUrl?` <a href="${esc(team().ruleSourceUrl)}" target="_blank" rel="noopener">Official source ↗</a>`:''}${team().localRulesNote?`<br><b>Local override:</b> ${esc(team().localRulesNote)}`:''}</div>
  ...
  function renderDocumentModal(){
    return modalShell('Upload team document',`<form id="documentForm"><div class="field"><label>File</label><input name="file" type="file" required accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.txt,.jpg,.jpeg,.png,.webp"></div><div class="form-grid"><div class="field"><label>Category</label><select name="category"><option>League Rules</option><option>Team Handbook</option><option>Schedule</option><option>Forms</option><option>Practice Resource</option><option>Medical / Safety</option><option selected>General</option></select></div><div class="field"><label>Share with</label><select name="visibility"><option value="team">Entire team</option><option value="guardians">Guardians</option><option value="coaches">Coaches only</option></select></div><div class="field span-2"><label>Description</label><textarea name="description" placeholder="What is this document for?"></textarea></div></div><div class="notice compact"><strong>Prototype storage:</strong> document bytes are kept in this browser's IndexedDB instead of LocalStorage. Production will move files to encrypted object storage with role-based access.</div><div class="modal-actions"><button type="button" class="secondary-btn" id="cancelModal">Cancel</button><button class="primary-btn">Upload & share</button></div></form>`);
  }
  ...
  function renderNotificationsModal(){
    return modalShell('Notifications',`<div class="empty-state"><strong>Notification center foundation</strong>Weather changes, schedule updates, team announcements and direct messages will appear here once realtime accounts and push delivery are connected.</div><div class="modal-actions"><button class="primary-btn" id="cancelModal">Done</button></div>`);
  }
  ...
  async function saveDocumentFromForm(e){
    e.preventDefault();const f=new FormData(e.currentTarget);const file=f.get('file');if(!(file instanceof File)||!file.size){showToast('Choose a document to upload');return;}if(file.size>25*1024*1024){showToast('Document limit is 25 MB');return;}
    ...
    try{
      if(team().remoteId&&window.TeamAppCloud?.session&&window.TeamAppCloud?.uploadDocument){showToast('Uploading secure team document…');const uploaded=await window.TeamAppCloud.uploadDocument(file,meta);state.documents.push(uploaded);save();modal=null;render();showToast('Document shared with team');return;}
