...
async function renderManager(container){
  const [list,invites]=await Promise.all([members(),invitations()]);const actorRole=role(),me=currentUser(),canAdmin=['owner','admin'].includes(actorRole);const roleOptions=['admin','coach','assistant_coach','manager','guardian','member','readonly'];
...
function enhanceAccess(){const sheet=accessSheet();if(!sheet||sheet.querySelector('#teamAppAccessHardening'))return;const box=document.createElement('section');box.id='teamAppAccessHardening';box.className='teamapp-access-hardening';box.innerHTML='<div class="separator"></div><button class="secondary-btn cloud-wide" id="manageAccessHardeningBtn">Manage current members & invitations</button><div id="accessHardeningBody"></div>';sheet.appendChild(box);box.querySelector('#manageAccessHardeningBtn').addEventListener('click',async e=>{const btn=e.currentTarget,body=box.querySelector('#accessHardeningBody');try{busy(btn,true,'Loading access…');await renderManager(body);btn.hidden=true;}catch(err){body.innerHTML=`<div class="cloud-error">${esc(err.message)}</div>`;}finally{busy(btn,false);}});}

function enhanceLogin(){
  const form=document.querySelector('#cloudOverlay #cloudAuthForm');if(!form||form.dataset.passwordRecovery==='1')return;form.dataset.passwordRecovery='1';
  const btn=document.createElement('button');btn.type='button';btn.className='secondary-btn cloud-wide';btn.id='forgotPasswordBtn';btn.textContent='Forgot password?';form.insertAdjacentElement('afterend',btn);
  btn.addEventListener('click',async()=>{const email=form.querySelector('[name="email"]')?.value?.trim();if(!email){form.querySelector('[name="email"]')?.focus();alert('Enter your email address first.');return;}try{busy(btn,true,'Sending recovery…');await requestPasswordReset(email);alert('Password recovery instructions were requested. Check your email.');}catch(error){alert(error.message);}finally{busy(btn,false);}});
}

function enhance(){enhanceAccess();enhanceLogin();}
const observer=new MutationObserver(enhance);
function start(){observer.observe(document.body,{childList:true,subtree:true});enhance();}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
window.TeamAppCloudAdmin={members,invitations,updateRole,removeMember,transferOwner,revokeInvitation,requestPasswordReset};
